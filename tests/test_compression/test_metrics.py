...
def test_log_metrics(compression_tracker):
    ...
    metrics = compression_tracker.log_metrics(step=100, tokens=tokens, logits=logits, loss=loss)
...
def test_log_metrics_with_activations(compression_tracker):
    ...
    metrics = compression_tracker.log_metrics(
        step=100, tokens=tokens, logits=logits, loss=loss, activations=activations
    )
